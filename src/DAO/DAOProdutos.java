...
public class DAOProdutos extends ConexaoMySql {
    ...
    public ModelProdutos getProdutosDAO(int pCodigo) {
        ...
        try {
            ...
            while (this.getResultSet().next()) {
                ...
                modelProdutos.setValorCusto(this.getResultSet().getDouble(5));
                modelProdutos.setCodigoBarras(this.getResultSet().getString(6));
                modelProdutos.setEstoque(this.getResultSet().getFloat(7));
                modelProdutos.setUnidadeMedida(this.getResultSet().getInt(8));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * @param pModelProdutos return boolean
     */
    public boolean atualizarProdutosQuantidadeValorDAO(ModelProdutos pModelProdutos) {
        try {
            this.conectar();
            int sizeLista = pModelProdutos.getListaModelProdutoses().size();
            for (int i = 0; i < sizeLista; i++) {
                this.executarUpdateDeleteSQL(
                        "UPDATE produtos SET "
                        + "estoque = '" + pModelProdutos.getListaModelProdutoses().get(i).getEstoque() + "',"
                        + "valor_custo = '" + pModelProdutos.getListaModelProdutoses().get(i).getValorCusto() + "',"
                        + "valor = '" + pModelProdutos.getListaModelProdutoses().get(i).getValor() + "'"
                        + " WHERE "
                        + "codigo = '" + pModelProdutos.getListaModelProdutoses().get(i).getCodigo() + "'"
                        + ";"
                );
